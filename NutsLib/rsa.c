...
{
	NN_DIGIT mod_t[2 * MAX_NN_DIGITS];    // ģ�����м����
...
void NN_ModExp (NN_DIGIT *a, NN_DIGIT *b, NN_DIGIT *c, uint32_t cDigits, NN_DIGIT *d, uint32_t dDigits)
{
	...
	uint32_t ciBits, j, s;
	NN_DIGIT ci;
	NN_DIGIT bPower[3][MAX_NN_DIGITS], Exp_t[MAX_NN_DIGITS];   // ָ�������м����
...
NN_DIGIT pc[MAX_NN_DIGITS];           // ��Կ������������
	NN_DIGIT pe[MAX_NN_DIGITS];           // ��Կָ��
	NN_DIGIT pm[MAX_NN_DIGITS];           // ��������
	NN_DIGIT pn[MAX_NN_DIGITS];           // ��Կģ
							     ...
							     R_RSA_PRIVATE_KEY *privateKey)     /* RSA private key */
{
	...
	NN_DIGIT c[MAX_NN_DIGITS];            // ˽Կ������ʱ�������㻺����
	NN_DIGIT cP[MAX_NN_DIGITS];           // cģp�������
	NN_DIGIT cQ[MAX_NN_DIGITS];           // cģq�������
	NN_DIGIT dP[MAX_NN_DIGITS];           // ˽Կָ��p����
	NN_DIGIT dQ[MAX_NN_DIGITS];           // ˽Կָ��q����
	NN_DIGIT mP[MAX_NN_DIGITS];           // mP = cP^dP mod p
	NN_DIGIT mQ[MAX_NN_DIGITS];           // mQ = cQ^dQ mod q
	NN_DIGIT n[MAX_NN_DIGITS];            // ˽Կģ
	NN_DIGIT p[MAX_NN_DIGITS];            // ˽Կp����
	NN_DIGIT q[MAX_NN_DIGITS];            // ˽Կq����
	NN_DIGIT qInv[MAX_NN_DIGITS];         // ˽Կp��q����ط���
	NN_DIGIT t[MAX_NN_DIGITS];            // ˽Կ������������
...
int32_t  rsa_public(void *outbuf, int32_t *outlen, const void *inbuf, int32_t inlen, const R_RSA_PUBLIC_KEY *pubkey)
{
	...
	uint8_t pkcsBlock[MAX_RSA_MODULUS_LEN]; // ��Կ����ʱ������ʱ���������
...
int32_t  rsa_private(void *outbuf, int32_t *outlen, const void *inbuf, int32_t inlen, const R_RSA_PRIVATE_KEY *prikey)
{
	...
	uint8_t pkcsBlock[MAX_RSA_MODULUS_LEN]; // ˽Կ����ʱ������ʱ���뻺����
